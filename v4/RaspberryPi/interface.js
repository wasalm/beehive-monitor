...
module.exports = {
	...
	displayWeatherInfo: async (dev, id, devices) => {
		...
		for (let i = 0; i < devices.length; i++) {
			if (devices[i].type == CONSTANTS.DEVICES.BME280 || devices[i].type == CONSTANTS.DEVICES.QMP6988) {
	...
	displayWeights: async (dev, id, currentArduino, devices) => {
		await dev.emptyScreen(id);
		await dev.writeScreen(id, 3, 1, "Weights #" + (currentArduino + 1));
		...
		let n = 5;
		for (let i = 0; i < devices.length; i++) {
			if (devices[i].type == CONSTANTS.DEVICES.HX711 && devices[i].arduino == currentArduino) {
	...
	displayTemperatures: async (dev, id, currentArduino, devices) => {
		await dev.emptyScreen(id);
		await dev.writeScreen(id, 0, 1, "Temperatures  #" + (currentArduino + 1));
		...
		let n = 5;
		for (let i = 0; i < devices.length; i++) {
			if (devices[i].type == CONSTANTS.DEVICES.DS18B20 && devices[i].arduino == currentArduino) {
				await dev.writeScreen(id, 0, n, devices[i].port);
